...
class Metaclass_MotorCommands(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('custom_vtol_plugin')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'custom_vtol_plugin.msg.MotorCommands')
            logger.debug(
...
class MotorCommands(metaclass=Metaclass_MotorCommands):
    ...
    def __init__(self, **kwargs):
        ...
        if 'motor_values' not in kwargs:
            self.motor_values = numpy.zeros(4, dtype=numpy.int16)
        else:
            self.motor_values = kwargs.get('motor_values')
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if any(self.motor_values != other.motor_values):
            return False
        return True
    ...
    def motor_values(self):
        """Message field 'motor_values'."""
        return self._motor_values

    @motor_values.setter
    def motor_values(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.int16, \
                "The 'motor_values' numpy.ndarray() must have the dtype of 'numpy.int16'"
            assert value.size == 4, \
                "The 'motor_values' numpy.ndarray() must have a size of 4"
            self._motor_values = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'motor_values' field must be a set or sequence with length 4 and each value of type 'int' and each integer in [-32768, 32767]"
        self._motor_values = numpy.array(value, dtype=numpy.int16)
